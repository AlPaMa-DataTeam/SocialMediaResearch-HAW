...
## Data processing
```{r}
twitterUser2 = mainData %>% 
  filter(Kategorie != "EU-Parlament") %>% # Switch between EU only and non EU by changing the != to ==
  filter(!is.na(SM_Twitter_user)) %>% 
  mutate(SM_Twitter_id = as.character(SM_Twitter_id))


#Test ob User IDs richitg sind. Klappt noch nicht so ganz
users = twitterList = c(twitterUser$SM_Twitter_user)
usersByName = lookup_users(users)
usersByName %>% 
  select(user_id)
test = usersByName %>% 
  full_join(twitterUser)
  #filter(!is.na(user_id)) %>% 
  #select(user_id, SM_Twitter_id)
```
Plots:
